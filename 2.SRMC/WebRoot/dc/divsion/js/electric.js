...
function initSndLevel(target,homeId){
	if('#container01' == homeId){
		window.location.href = basePath + '/elecsnd?name='+encodeURI(encodeURI(target.series.name))+'&month='+(target.x + 1)+'&year='+year;
	}else if('#container02' == homeId){
		window.location.href = basePath + '/elecsnd?name='+encodeURI(encodeURI(target.name))+'&month='+(target.x + 1)+'&year='+year;
	}else{
		return false;
	}
}
