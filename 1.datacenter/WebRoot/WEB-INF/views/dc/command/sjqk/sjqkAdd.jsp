...
<form action=""  method="post"  id="form1">
	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="editTable reeditTable" align="center" id="table01">
		...
		<input type="hidden" name="worksType" id="worksType" value="8"/>
		<tr>
			<td colspan="4">
	    ...
	    <tr>
	    	...
	    	<td colspan="3">
	    		<div class="word_5">
                	<p>于<input class="borderBtm" style="width:100px;" name="beginYear" id="beginYear" value="${vo.beginYear }" />年<input class="borderBtm" style="width:40px;" name="beginMonth" id="beginMonth" value="${vo.beginMonth }" />月至<input class="borderBtm" style="width:100px;" name="endYear" id="endYear" value="${vo.endYear }" />年<input class="borderBtm" style="width:40px;" name="endMonth" id="endMonth" value="${vo.endMonth }" />月期间，
根据合同条款第<input class="borderBtm" style="width:40px;" name="termsNum" id="termsNum" value="${vo.termsNum }" />条约定：<input class="borderBtm" style="width:400px;" name="termsContent" id="termsContent" value="${vo.termsContent }" /><br/><br/>我方申请合同付款，
...
$.ajax({
	url:"<%=basePath%>command/queryByOrgNameD",
	type:"post",
	data:{orgName : name},
...
window.onload = function() {
	$("#termsContent").keyup(function(){
	   ...
	   if(len > 360){
	    	$(this).val($(this).val().substring(0,360));
	   }
	});
}
...
function ev_required_item(){
		...
		if(bindChange("#endYear",4,"结束年份不得超过4个字")){
			return false;
		}
		if(bindChange("#endMonth",2,"结束月份不得超过2个字")){
			return false;
		}
		if(bindChange("#termsNum",4,"合同条款条数不得超过4个字")){
			return false;
		}
		if(bindChange("#termsContent",360,"合同条款内容不得超过360个字")){
			return false;
		}
