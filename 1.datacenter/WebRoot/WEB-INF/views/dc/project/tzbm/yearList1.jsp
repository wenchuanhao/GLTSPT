...
<div class="gl_import_m" >
  	...
  	<div class="tabpages" >
    	...
    	<div class="otherButtons r">
    		...
    		<a class="btn_common01" href="javascript:location.href='<%=basePath%>jsxm/tzbmList?id=${jsxmId}';" ><img src="/SRMC/dc/images/btnIcon/back.png" /><span>返回</span></a>
		</div>    	
  	</div>
...
function ev_year(id){
	document.forms[0].action="<%=basePath%>tzbm/useYear1?id="+id+"&parentId=${param.parentId}";
...
function ev_add(id){
	var url = "<%=basePath%>tzbm/yearAdd1?parentId=${param.parentId}&id="+id;
	ev_submit(url);
...
function ev_delete(id){
	if(confirm("确定删除？")){
		document.forms[0].action="<%=basePath%>tzbm/deleteYear1?id="+id+"&parentId=${param.parentId}";
		document.forms[0].submit();
	}
}
...
function ev_refresh(type){
	if(type == "Y"){
		alert("刷新成功");
		document.forms[0].action = "<%=basePath%>tzbm/yearList1?parentId=${param.parentId}";
		document.forms[0].submit();	
	}
}
</script>
