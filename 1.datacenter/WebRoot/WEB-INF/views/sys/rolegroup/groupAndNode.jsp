...
function delRoleUser(roleuserId){
   ...
   var pageSize2=document.getElementById("pageSize2").value;
    jQuery.ajax({
		type:"POST",
		url:"<%=basePath%>sys/rolegroup/delGroupUser",
		...
		success:function(result){	
		  if(result=='1'){
			...
			window.parent.document.getElementById("toGetUser").src="<%=basePath%>sys/rolegroup/userList?flowNodeId="+flowNodeId+ "&pageSize1="+pageSize1;
			window.parent.document.getElementById("userFlowActor").src="<%=basePath%>sys/rolegroup/getAllConfigUser?flowNodeId="+flowNodeId + "&pageSize2="+pageSize2+ "&pageIndex="+ pageIndex;
		  }if(result=='0'){
		    alert("删除失败!");
		  }
		},
          ...
          <th>操作</th>
		        ...
		        <tr <c:if test="${i.index%2 != 0}">class="tab_c_tr01"</c:if>>
	               ...
	               <td>
	                  ...
	                  <a href="javascript:delRoleUser('${items.userId }');">删除</a>
