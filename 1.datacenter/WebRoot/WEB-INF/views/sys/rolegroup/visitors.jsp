...
<body class="main_bg">
  <form name="form" method="post" action="<%=basePath%>sys/rolegroup/getUser/1/${roleGroupId}">
  	<input type="hidden"  id="roleGroupId" name="roleId" value="${roleGroupId}"/>
    <div class="mm_main_bj">
     ...
     <div class="mm_main_bd01">
	      <div class="mm_m_bd_a">
	        ...
	        <span class="mm_m_bd_c">已分配用户</span>
	      </div>
	      ...
	      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="main_table01">
			...
			<c:forEach items="${ITEMPAGE.items}" var="user">
				<tr <c:if test="${i.index%2 != 0}">class="main_table04_bg"</c:if> style="cursor: hand;" >
		          <td><input type="checkbox" name="checkbox" id="checkbox" value="${user.userId}" onclick="doAssign('0',this)"/></td>
		          <td>${user.account}</td>
		          <td>${user.userName}</td>
