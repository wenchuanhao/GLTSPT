...
<script type="text/javascript">
  	 function  setNodeName(nodeId,nodeName,liId){
	   	...
	   	document.getElementById("toGetUser").src="<%=basePath%>sys/rolegroup/userList?flowNodeId=" + nodeId +"&organizationId="+organizationId;
		document.getElementById("userFlowActor").src="<%=basePath%>sys/rolegroup/getAllConfigUser?flowNodeId=" + nodeId;
		document.getElementById("toTree").src="<%=basePath%>sys/rolegroup/getOrganizationTree?flowNodeId=" + nodeId;
		//查询角色组包含的角色
		jQuery.post("<%=basePath%>sys/rolegroup/queryConfigRole", {"roleGroupid":nodeId}, function(data){
			$("#roleNameList").html(data);
			$("#roleNameList").prop("title",data);
		});
  	}
  	$(document).ready(function(){
  	 ...
  	 //查询
     function doSerachs(){
       $("#userForm").attr("action","<%=basePath%>sys/rolegroup/userList");
       form.method="post";
...
<body class="bg_c_g">
	<div class="gl_m_r_nav">当前位置 : 系统管理 > 角色管理 > 给角色组赋予用户</div>
	...
	<table class="tuand_tab01" border="0" cellspacing="0" cellpadding="0" id="showTable">
		...
		<input type="hidden" value="${pageSize2}" id="pageSize2"
			name="pageSize2" />
		<tr>
			<th style="width:15%">第一步: 选择角色组</th>
		...
		<tr>
			<td valign="top">
				<ul class="tuand_ul01">
					...
					<c:forEach items="${sysRoleGroupList}" var="flowNode" varStatus="i">
						<li id="ones${i.index+1}"
							onclick="setNodeName('${flowNode.roleGroupid}','${flowNode.roleGroupname}','ones${i.index+1}')"
							title="${flowNode.roleGroupname}">
							<p class="tuand_li_l">${flowNode.roleGroupname}(${flowNode.roleGroupcode})</p></li>
					</c:forEach>
				</ul>
				<div class="ge01"></div>
			</td>
			...
			<td colspan="2" valign="top">
				<table width="100%" class="tuan02_bao01" border="0" cellspacing="0"
					cellpadding="0">
					<tr>
						<td width="85" style="padding-left:5px;">角色组名称:</td>
						<td><p class="tuand_li_l" id="nodeNameText"></p>
						</td>
					</tr>
					<tr>
						<td width="85" style="padding-left:5px;">包含角色:</td>
						<td><p class="tuand_li_l" id="roleNameList"></p>
						</td>
					</tr>
				</table>
				<table width="100%" class="tuan02_bao01" border="0" cellspacing="0"
				...
				<form id="userForm" name="form" action="<%=basePath%>sys/rolegroup/userList">
						...
						<tr>
							<td width="50%" valign="top">
								<iframe id="toTree"
									...
									marginheight="0"
									src="<%=basePath%>sys/rolegroup/getOrganizationTree"></iframe>
							...
							<td width="50%">
								...
								<div class="tuan_d02">
									<iframe id="toGetUser" name="toGetUser"
										style="height: 450px; width: 100%; visibility: inherit;"
										scrolling="no" frameborder="0" marginwidth="0"
										marginheight="0" src="<%=basePath%>sys/rolegroup/userList"></iframe>
								</div>
								<div class="ge01"></div>
								</div>
							</td>
				...
				<table class="tuand_tab01" border="0" cellspacing="0"
					...
					<div class="tuan_d02">
						...
						<iframe id="userFlowActor" name="userFlowActor"
							...
							src="<%=basePath%>sys/rolegroup/getAllConfigUser"> </iframe>
					</div>
				</table>
			</td>
		</tr>
	</table>
	<c:if test="${empty roleGroupid }">
		<div class="gl_ipt_03">
