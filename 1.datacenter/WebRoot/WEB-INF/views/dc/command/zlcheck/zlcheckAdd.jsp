...
<!--word.css -->
<link href="/SRMC/dc/css/word.css" rel="stylesheet" type="text/css" />
<!--公共JS -->
	...
	<div class="gl_import_m">
		...
		<form action="" method="post" id="form1">
			<table width="99%" border="0" cellspacing="0" cellpadding="0"
				...
				<input type="hidden" name="worksType" id="worksType" value="6" />
				
				    <tr>
			         <td align="center" colspan="4">
				      <div class="contract_head" >
					     <h1 class="contract_title"  >非采购订单-工程建设类请款审批表（检测、勘察）</h1>
				      </div>
			         </td>
		            </tr>	
			    ...
			    <tr>
					...
					<td><input name="projectName" id="projectName" type="text"
						...
						placeholder="请输入投资项目名称" value="${vo.projectName}" /></td> --%>
						<th width="20%" align="right"><b>*</b>投资项目名称：</th>
...
根据合同条款<input class="borderBtm" style="width:40px;" name="termsNum" id="termsNum" value="${vo.termsNum }" />条：<textarea class="borderBtm" style="width:770px; height:30px;" name="termsContent" id="termsContent" value="${vo.termsContent }"></textarea>。<br/><br/>本期应扣除金额为<input class="borderBtm" style="width:140px;" name="deductAmount" id="deductAmount" value="<fmt:formatNumber value='${vo.deductAmount }' pattern="##.##" minFractionDigits="2"></fmt:formatNumber>" placeholder="￥" />元，
...
$.ajax({
	url:"<%=basePath%>command/queryByOrgNameB",
	type:"post",
	data:{orgName : name},
...
function ev_dosubmit(){
	$("#form1").ajaxSubmit({
       	url:"<%=basePath%>command/zlcheckSave",
    	dataType:'json',
        async:true,
        type:'POST',
...
function ev_required_item(){
		...
		if(bindChange("#projectName",28,"投资项目名称不得超过28个字")){
			return false;
		}
		if(bindChange("#projectCode",30,"投资项目编号不得超过30个字")){
			return false;
		}
		if(bindChange("#constructionName",28,"乙方单位名称不得超过28个字")){
			return false;
		}
		...
		if(bindChange("#workContent",40,"工作内容不得超过40个字")){
			return false;
		}
