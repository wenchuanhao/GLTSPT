...
(function($){
    var root={},
        ...
        tools = {
            getActor:function(setting,processMode){
                var _actors=[],params = {
                    workorderId:setting.worksheetWorkorderId,
                    insactorId:setting.insactorId,
                    worksheetOpertypeSonId:setting.worksheetOpertypeSonId,
    ...
    view = {
            ...
            if(!flowTools.checkNeedActor(cSetting,_processMode.flowModeValue)){
                ...
                switch (_processMode.flowModeStatus){
                    case _consts.fenfa :
                        ...
                        break;
                    case _consts.backFenfa :
                    	view.backFenfa(cSetting,_processMode); //驳回到分发执行
                    	break;
        ...
        appendCheckBoxActor:function(setting,actors,ischecked){
            //分发
            //alert("暂时没有分发");
            var selectHtml = [];
            var actor = tools.clone(_actor);
            for(var i=0;i<actors.length;i++){
                $.extend(true,actor,actors[i]);
                if(ischecked==true) //勾选
                	selectHtml.push(" <label><input type=\"checkbox\" checked value=\"",actor.userId,"\" />",actor.userName,"</label> ");
                else
                	selectHtml.push(" <label><input type=\"checkbox\" value=\"",actor.userId,"\" />",actor.userName,"</label> ");
            }
            setting.processActorObj.html(selectHtml.join(''));
        },
        ...
        showActor:function(setting,processMode){
            ...
            if(tools.isArray(actorSelect) && actorSelect.length > 0) {
                ...
                setting.processActorObj.html("<font color=\"red\">该现网操作类型尚未配置流程节点处理人，请联系管理员进行业务流程配置</font>");
        ...
        fenfa:function(setting,processMode){
            ...
            if(tools.isArray(actorSelect) && actorSelect.length > 0) {
                /////view.appendSelectActor(setting,actorSelect);
                view.appendCheckBoxActor(setting,actorSelect);
            } else if(actorSelect == "" || actorSelect.length == 0){
            														//未配置处理人
            	setting.processActorObj.html("<font color=\"red\">该现网操作类型尚未配置流程节点处理人，请联系管理员进行业务流程配置</font>");
            } else if(actorSelect == "fail") {
        ...
        backFenfa:function(setting,processMode){
        	view.loading(setting.processActorObj,"加载中...");
            var actorSelect = tools.getActor(setting,processMode);
            if(tools.isArray(actorSelect) && actorSelect.length > 0) {
                /////view.appendSelectActor(setting,actorSelect);
                view.appendCheckBoxActor(setting,actorSelect,true);
            } else if(actorSelect == "" || actorSelect.length == 0){
            														//未配置处理人
            	setting.processActorObj.html("<font color=\"red\">该现网操作类型尚未配置流程节点处理人，请联系管理员进行业务流程配置</font>");
            } else if(actorSelect == "fail") {
    ...
    $.fn.flowProcess = {
        init:function(fSetting,flowModes){
            ...
            var returnFlowTools = {
                 ...
                 */
                checkActor:function(){
                    if(settings.processActorObj.find("input[type=hidden],select,:checkbox:checked").length == 0 && flowTools.checkNeedActor(settings,_cache.checkedProcessMode.flowModeValue)){
                        return false;
                    }
                    return true;
                },
                ...
                getActor:function(){
                    var actor_ids = [];
                    settings.processActorObj.find("input[type=hidden],select,:checkbox:checked").each(function(){
                ...
                reset:function(){
                    //清空页面痕迹
                    settings.processModeObj.empty();
                    settings.processActorObj.empty();
                    settings.copyToObj.empty();
                },
                
                /**
                 * 获取配置好的处理人 id数组
                 */
                getCfgActors:function(transitionId){
                	var users = tools.getActor( settings , {flowModeValue:transitionId} );
                	var ids = [];
                	for(var i in users){
