...
<body>
	...
	<div class="gl_import_m">
		...
		<form action="" method="post" id="form1">
			<table width="99%" border="0" cellspacing="0" cellpadding="0"
				...
				<input type="hidden" name="worksId" id="worksId"
					value="${vo.worksId}" />
				<input type="hidden" name="worksType" id="worksType" value="5" />
								
					<tr>
			        <td align="center" colspan="4">
				      <div class="contract_head">
					     <h1 class="contract_title">非采购订单-工程建设类请款审批表（施工类）</h1>
				      </div>
			        </td>
		            </tr>	
					<tr>
					...
					<td><input name="contractName" id="contractName" type="text" <c:if test="${not empty vo.contractName }">readonly="readonly"</c:if>
						class="text01"  style="width:300px;"onfocus="autoCompletesProject(this)"
						placeholder="请输入合同名称" value="${vo.contractName}" />
						</td> --%>
					<th width="20%" align="right"><b>*</b>合同名称：</th>
					<td>
					...
					<td>
						<input name="contractCode" id="contractCode" <c:if test="${not empty vo.contractCode }">readonly="readonly"</c:if>
						 value="${vo.contractCode}" onfocus="autoCompletesProject(this)"
						type="text" class="text01" style="width:300px;"
						placeholder="请输入合同编号">
						</td>
				</tr>
				
				<tr>
					<th width="20%" align="right">框架合同名称：<br/>（非框架合同此行不填）</th>
					<td><input name="frameworkName" id="frameworkName" type="text"
						class="text01"  style="width:300px;"
						placeholder="请输入合同名称" value="${vo.frameworkName}" /></td>
					<th align="right">框架合同编号:<br/>（非框架合同此行不填）</th>
					<td><input name="frameworkCode" id="frameworkCode"
						 value="${vo.frameworkCode}"
						type="text" class="text01" style="width:300px;"
			    ...
			    <tr>
					...
					<td><input name="projectName" id="projectName" type="text"
						...
						placeholder="请输入投资项目名称" value="${vo.frameworkName.projectName}" /></td> --%>
					...
					<td><input name="projectCode" id="projectCode"
						 value="${vo.projectCode}" onfocus="autoProject(this)"
						type="text" class="text01" style="width:300px;"
						placeholder="请输入投资项目编号"></td>
				</tr>
			
				<tr>
					<th width="20%" align="right"><b>*</b>乙方单位：</th>
					<td><input name="constructionName" id="constructionName" type="text"
						class="text01"  style="width:300px;"
						placeholder="请输入乙方单位名称" value="${vo.constructionName}" /></td>
					<th align="right"><b>*</b>合同金额:</th>
					<td><input name="contractAmount" id="contractAmount"
						 value="<fmt:formatNumber value='${vo.contractAmount}' pattern="##.##" minFractionDigits="2"></fmt:formatNumber>"
						type="text" class="text01" style="width:300px;"
						placeholder="请输入合同金额"></td>
				</tr>
		    ...
		    <td>
		    	<div class="contract_chex" style="border:1px solid #FFFFFF;border-radius: 5px;width: 305px;" " id="contract_chex">
						<span>
						...
						<span>
							...
							<label class="chex" for="qkTypex">其他 &nbsp&nbsp<input id="qkTypedec" name="qkTypedec" value="${vo.qkTypedec}"  class="borderBtm" style="width:100px;" /></label>
						</span>
		    ...
		    <input  name="paymentTerms"  id= "paymentTerms"  value="${vo.paymentTerms}" type="text" class="text01" style="width:300px;" placeholder="请输入付款条款" />
				...
				<tr>
					...
					<td><input name="applicationAmount" id="applicationAmount"
						value="<fmt:formatNumber value='${vo.applicationAmount}' pattern="##.##" minFractionDigits="2"></fmt:formatNumber>" type="text" class="text01"
						style="width:300px;" placeholder="请输入乙方申请金额" /> 
				</tr>
	    ...
	    <tr>
	    	...
	    	<td colspan="3">	
	    		<div class="word_5">
                	<p>于<input class="borderBtm" style="width:60px;" name="beginYear" id="beginYear" value="${vo.beginYear }" />年<input class="borderBtm" style="width:40px;" name="beginMonth" id="beginMonth" value="${vo.beginMonth }" />月期间，我方已完成工程款额&nbsp
                	<input name="engineerAmount" id="engineerAmount" value="<fmt:formatNumber value='${vo.engineerAmount }' pattern="##.##" minFractionDigits="2"></fmt:formatNumber>"  placeholder="￥"class="borderBtm" style="width:140px;" />元，
...
根据合同条款<input class="borderBtm" style="width:40px;" name="termsNum" id="termsNum" value="${vo.termsNum }" />条：<textarea class="borderBtm" style="width:770px; height:30px;" name="termsContent" id="termsContent" value="${vo.termsContent }"></textarea>。
<br/><br/>本期应扣除金额为<input class="borderBtm" style="width:140px;" name="deductAmount" id="deductAmount" value="<fmt:formatNumber value='${vo.deductAmount }' pattern="##.##" minFractionDigits="2"></fmt:formatNumber>" placeholder="￥" />元，
...
$.ajax({
	url:"<%=basePath%>command/queryByOrgNameA",
	type:"post",
	data:{orgName : name},
...
function ev_dosubmit(){
	$("#form1").ajaxSubmit({
       	url:"<%=basePath%>command/zlbuildSave",
    	dataType:'json',
        async:true,
        type:'POST',
...
function ev_required_item(){
		...
		if(bindChange("#qkTypedec",3,"请款类型描述不得超过6个字")){
			return false;
		}
		if(bindChange("#paymentTerms",30,"对应合同付款条款不得超过30个字")){
			return false;
		}
		...
		if(bindChange1("#engineerAmount",Math.pow(10,15),"工程款额不得超过15位数")){
			return false;
		}
		...
		if(bindChange1("#deductAmount",Math.pow(10,15),"本期应扣金额不得超过15位数")){
			return false;
		}
		if(bindChange1("#payAmount",Math.pow(10,15),"本期申请支付金额不得超过15位数")){
			return false;
		}
		...
		//工程款额填入的必须为数字
		var engineerAmount = $("#engineerAmount");
		var value = document.getElementById("engineerAmount").value;
		var reg=/^\d+(\.\d+)?$/; //能输入数字和小数点
		if(engineerAmount.val() != "" && reg.test(value)!=true){
			bindC(engineerAmount);
			k1 = false;
		}
		//本期应扣除金额填入的必须为数字
