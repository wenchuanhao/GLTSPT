...
.pro_cal{padding-bottom:5px; display:inline-block; border-bottom:1px solid #FFFFFF;}
</style> 

<div class="contract_wraper">
	<div class="contract" id="contract" style="height:auto;">
		<div class="contract_head">
			<h1 class="contract_title">非采购订单-工程建设类请款审批表<i>（检测、勘察）</i></h1>
		 ...
		 -->
            ...
            <tr class="noBorder">
                ...
                <div class="word_5">乙方单位申请事由：
                	...
                	年
                	<i  class="borderBtm" style="width:60px;"><c:if test="${empty vo.beginMonth }">&nbsp;</c:if><c:if test="${not empty vo.beginMonth }">${vo.beginMonth }</c:if></i>
                	月期间，我方已完成
                	<i class="<c:if test='${empty vo.workContent }'>divcss5-x5-null</c:if><c:if test='${not empty vo.workContent }'>divcss5-x5</c:if>" style="min-width:100px;"><c:if test="${empty vo.workContent }">&nbsp;</c:if><c:if test="${not empty vo.workContent }">${vo.workContent }</c:if></i>
                	工作内容，根据合同条款
                	<i class="borderBtm" style="min-width:60px;"><c:if test="${empty vo.termsNum }">&nbsp;</c:if><c:if test="${not empty vo.termsNum }">${vo.termsNum }</c:if></i>
                	 条：
                	<i class="<c:if test='${empty vo.termsContent }'>divcss5-x5-null</c:if><c:if test='${not empty vo.termsContent }'>divcss5-x5</c:if>" style="min-width:100px;"><c:if test="${empty vo.termsContent }">&nbsp;</c:if><c:if test="${not empty vo.termsContent }">${vo.termsContent }</c:if></i>
                	。本期间应扣金额为
           ...
           <tr class="noBorderR noBorder">
                ...
                <div>造价公司意见(审批时限: 500万以下3个工作日, 500万以上5个工作日)：
                ...
                <p>二、根据合同条款第<b class="borderBtm" style="width:100px;">&nbsp;</b>条：<b class="borderBtm" style="width:400px;">&nbsp;</b>。</p>
            ...
            <tr class="noBorder">
				<td colspan="4" class="contract_item" style="text-align:left;">
                <div class="pad-btm10">专业工程师意见</div>
                <div class="contract_chex">
						<span>
