...
<div class="gl_import_m">
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="editTable reeditTable" align="center" id="table01">
	    ...
	    <tr>
		    ...
		    <a onclick="ev_selectzxmHtList()" style="cursor: pointer;color: blue;" title="选择合同">选择</a>
		    </c:if>
...
function ev_list(){
window.location.href='<%=basePath%>zxm/htList?id=${zxmId}';
}
    //兼容谷歌  
		...
		if($("#fileName").val() != ""){
			...
			$("input[id='column21Id']").each(function(){ 
    	  	...
    	  	$("#uploadFiles1").uploadifive('upload');
				...
				document.forms[0].action="<%=basePath%>zxmHt/saveOrUpdate1";
...
function ev_selectList(){
	//判断是不是IE浏览器
	if(isIE()){
		var returnValue = window.showModalDialog("<%=basePath%>zxm/selectList?source=add1",window,"dialogWidth:"+window.screen.width+"px;dialogHeight:"+window.screen.height+"px"); 
		setValues(returnValue);
	}else{
		//其他浏览器
		window.open("<%=basePath%>zxm/selectList?source=add1",window,"dialogWidth:"+window.screen.width+"px;dialogHeight:"+window.screen.height+"px");
	}
}

function setValues(returnValue){
	if(returnValue){
...
function ev_selectzxmHtList(){
	...
	//addzxmId(returnValue);
	//判断是不是IE浏览器
	if(isIE()){
		var returnValue = window.showModalDialog("<%=basePath%>zxmHt/selectList?source=zxmHt",window,"dialogWidth:"+window.screen.width+"px;dialogHeight:"+window.screen.height+"px"); 
		addzxmId(returnValue);
	}else{
		//其他浏览器
		window.open("<%=basePath%>zxmHt/selectList?source=zxmHt",window,"dialogWidth:"+window.screen.width+"px;dialogHeight:"+window.screen.height+"px");
	}
}

function addzxmId(returnValue){
	if(returnValue){
		var v = returnValue.split("_");
		window.location.href =  "<%=basePath%>zxmHt/add1?zxmId=${zxmId}&id="+v[0];
	}
}
</script>