...
<div class="contract_wraper">
	<div class="contract" id="contract" style="height:auto;">
		<div class="contract_head">
			<h1 class="contract_title">非采购订单-工程建设-请款审批表<i>（设计、专家顾问）</i></h1>
		 ...
		 -->
            ...
            <tr class="noBorder">
                ...
                <div class="word_5">乙方申请事由：
                	<p>于<i class="borderBtm" style="width:100px;"><c:if test="${empty vo.beginYear }">&nbsp;</c:if><c:if test="${not empty vo.beginYear }">${vo.beginYear }</c:if></i>年
                	<i class="borderBtm" style="width:40px;"><c:if test="${empty vo.beginMonth }">&nbsp;</c:if><c:if test="${not empty vo.beginMonth }">${vo.beginMonth }</c:if></i>月至
                	<i class="borderBtm" style="width:100px;"><c:if test="${empty vo.endYear }">&nbsp;</c:if><c:if test="${not empty vo.endYear }">${vo.endYear }</c:if></i>年
                	<i class="borderBtm" style="width:40px;"><c:if test="${empty vo.endMonth }">&nbsp;</c:if><c:if test="${not empty vo.endMonth }">${vo.endMonth }</c:if></i>月期间，根据合同条款第
                	...
                	<i class="<c:if test='${empty vo.termsContent }'>divcss5-x5-null</c:if><c:if test='${not empty vo.termsContent }'>divcss5-x5</c:if>" style="min-width:120px;" ><c:if test="${empty vo.termsContent }">&nbsp;</c:if><c:if test="${not empty vo.termsContent }">${vo.termsContent }</c:if></i> ，我方申请合同付款，我方本次申请支付金额为
                	<i class="borderBtm" style="min-width:100px;"><c:if test="${empty vo.payAmount }">&nbsp;</c:if><c:if test="${not empty vo.payAmount }"><fmt:formatNumber value="${vo.payAmount }" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber></c:if></i> 元。            
</p>
                </div>
                </td>
			</tr>
           ...
           <tr class=" noBorder">
                ...
                <br><p>于<i class="borderBtm" style="width:100px;">&nbsp;</i>年<i class="borderBtm" style="width:40px;">&nbsp;</i>月至<i class="borderBtm" style="width:100px;">&nbsp;</i>年<i class="borderBtm" style="width:40px;">&nbsp;</i>月期间，根据合同条款第<i class="borderBtm" style="width:40px;">&nbsp;</i>条约定：<i class="borderBtm" style="width:100px;">&nbsp;</i></p><br>
                <p>经复核，本次应支付金额<i class="borderBtm" style="width:100px;color:black">&nbsp;</i>元； </p>
                </td>
			</tr>
            <tr >
            ...
            <tr class="noBorder">
				<td colspan="4" class="contract_item" style="text-align:left;">
                <div class="pad-btm10">项目负责人意见：</div>
                <div class="contract_chex">
						<span>
