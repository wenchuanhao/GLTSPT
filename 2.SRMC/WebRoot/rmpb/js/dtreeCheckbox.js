// Node object
function Node(id, pid, name, checked, url, title, target, treecheckbox,
		checkdisabled, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
function dTree(objName) {
	...
	this.icon = {
		...
		root: '/SRMC/rmpb/images/treemenu/base.gif',
		folder: '/SRMC/rmpb/images/treemenu/folder.gif',
		folderOpen: '/SRMC/rmpb/images/treemenu/folderopen.gif',
		node: '/SRMC/rmpb/images/treemenu/page.gif',
		empty: '/SRMC/rmpb/images/treemenu/empty.gif',
		line: '/SRMC/rmpb/images/treemenu/line.gif',
		join: '/SRMC/rmpb/images/treemenu/join.gif',
		joinBottom: '/SRMC/rmpb/images/treemenu/joinbottom.gif',
		plus: '/SRMC/rmpb/images/treemenu/plus.gif',
		plusBottom: '/SRMC/rmpb/images/treemenu/plusbottom.gif',
		minus: '/SRMC/rmpb/images/treemenu/minus.gif',
		minusBottom: '/SRMC/rmpb/images/treemenu/minusbottom.gif',
		nlPlus: '/SRMC/rmpb/images/treemenu/inolines_plus.gif',
		nlMinus: '/SRMC/rmpb/images/treemenu/nolines_minus.gif'
...
dTree.prototype.add = function(id, pid, name, checked, url, title, target,
		treecheckbox, checkdisabled, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, checked, url,
			title, target, treecheckbox, checkdisabled, icon, iconOpen, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if ("//" == node.treecheckbox)
			str += '<img id="i' + this.obj + nodeId + '" src="'
					+ ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
		else {
			if (node.checked){
				if(node.id=="9999999"){
					str += '<input type="checkbox" checked="checked" name="'
						+ node.name + '" onClick="check(\'' + node.id + '\','
						+ node.treecheckbox + ')" id="9999999" '
						+ node.treecheckbox + ' value="' + node.id + '" '
						+ node.checkdisabled + ' pid="' + node.pid
						+ '"><img id="i' + this.obj + nodeId + '" src="'
						+ ((node._io) ? node.iconOpen : node.icon)
						+ '" alt="" />';
				}else{
					str += '<input type="checkbox" checked="checked" name="'
						+ node.name + '" onClick="check(\'' + node.id + '\','
						+ node.treecheckbox + ')" id="checkbox" '
						+ node.treecheckbox + ' value="' + node.id + '" '
						+ node.checkdisabled + ' pid="' + node.pid
						+ '"><img id="i' + this.obj + nodeId + '" src="'
						+ ((node._io) ? node.iconOpen : node.icon)
				...
				if(node.id=="9999999"){
				str += '<input type="checkbox" name="' + node.name
						+ '" onClick="check(\'' + node.id + '\','
						+ node.treecheckbox + ')" id="9999999" '
						+ node.treecheckbox + ' value="' + node.id + '" '
						+ node.checkdisabled + ' pid="' + node.pid
						+ '"><img id="i' + this.obj + nodeId + '" src="'
