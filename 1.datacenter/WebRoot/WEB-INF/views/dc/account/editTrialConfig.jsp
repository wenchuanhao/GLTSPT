...
<div class="taskCreatBox">
            ...
            <table cellpadding="0" cellspacing="5" class="taskCreate" width="100%">
       		...
       		<input type="hidden" name="departmentId" id="departmentId" value="${config.departmentId }"/>
       		<input type="hidden" name="department" id="department" value="${config.department }" />
       		<input type="hidden" name="costType" id="costType" value="${config.costType}"/>
       		<input type="hidden" name="cosId" id="cosId" value="${config.cosId}"/>
       		<input type="hidden" name="cosIdTemp" id="cosIdTemp" value="${config.cosId}"/>
       		<input type="hidden" name="depIdTemp" id="depIdTemp" value="${config.departmentId }"/>
	            <tr>
               <th width="35%"><em>*</em>费用类型：</th>
               <td>
			    <div class="checxTreeWrap"  style="width:170px;">
				    <select multiple style="width:200px" id="cosIdTmep" name="cosIdTmep" class="easyui-combotree" data-options="url:'<%=basePath%>/account/getCostType',method:'get'" >
				    <option selected="selected" >${config.costType }</option>
				    </select>
				    </div>
                </td>
            	</tr>
             	<tr>
	               ...
	               <td>
	               <div class="checxTreeWrap"  style="width:170px;">
				    <select multiple style="width:200px" id="departmenTmep" name="departmenTmep" class="easyui-combotree" data-options="url:'<%=basePath%>/account/getDepartment',method:'get'" >
				    <option selected="selected">${config.department }</option>
				    </select>
				    </div>
	               <%-- <input   onfocus="if(this.value=='请输入报账部门'){this.value='';};this.style.color='#333333';autoDepartment();" 
			                     onkeyup="clean()"
			                     onblur="if(this.value==''||this.value=='请输入报账部门'){this.style.color='#b6b6b6';}" placeholder="请输入报账部门" type="text" name="department" id="sel_02" style="width:160px;" value="${config.department}" class="text01" /> --%>
	               </td>
            	</tr>
             	<tr>
	               <th><em>*</em>初审会计：</th>
	               <td>
					...
					<c:forEach items="${queryerList}" var="item" varStatus="i">
						<a href="javascript:void(0);" class="" id="${item.userId }">
							...
							<input type="hidden" id="accounting" name="accounting" value="${item.userName }">
...
function save(){
    ...
    jQuery("#accountingId").val(jQuery("#accountingId").val());
    
	var departext = jQuery("#department").val();	
	var costext = jQuery("#costType").val();	
	
	if(jQuery.trim(costext) == ""){
		alert("请选择费用类型");
		return;
	}
	if(jQuery.trim(departext) == ""){
		alert("请选择部门");
		return;
	}
	...
	$.ajax({
		url:"<%=basePath%>account/updateTrialConfig",
    	dataType:'json',
        async:false,
        type:'POST',
...
$(function (){
	...
	j("#departmenTmep").combotree({
		onClick: function(node) {
			if (!j(this).tree('isLeaf', node.target)) {
				j('#departmenTmep').combo('showPanel');
			}
			
			jQuery("#department").val(j("#departmenTmep").combotree("getText"));	
			jQuery("#departmentId").val(j("#departmenTmep").combotree("getValue"));
		},
		...
		multiple:false//设置是否多选	
		   ...
		   onClick: function(node) {
			if (!j(this).tree('isLeaf', node.target)) {
				j('#cosIdTmep').combo('showPanel');
			}
			jQuery("#costType").val(j("#cosIdTmep").combotree("getText"));
			jQuery("#cosId").val(j("#cosIdTmep").combotree("getValue"));
		},
