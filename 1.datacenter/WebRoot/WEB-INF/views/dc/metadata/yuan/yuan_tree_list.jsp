...
var treeNodeObj;
    ...
    function beforeEditName(treeId, treeNode) {
    	...
    	//加载空的表单出来
    	$("#data").load("<%=basePath%>yuan/add?id="+treeNode.id,function(){
    ...
    function addHoverDom(treeId, treeNode) {
      ...
      if (btn) btn.bind("click", function(){
		    	...
		    	//加载空的表单出来
		    	$("#data").load("<%=basePath%>yuan/add?parentId="+treeNode.id,function(){
				    	obj2.val("");
				    	$("#parentId_tr01").show();
...
var obj2 ;
 ...
 $(document).ready(function(){
 	//加载空的表单出来
 	$("#data").load("<%=basePath%>yuan/add",function(){
		$("#parentId_tr01").hide();
		
 	 	// 将所有.ui-select实例化
 		$('.ui-select').ui_select();
 		obj2 = $('#parentId').data('ui-select');
...
function ev_save(){
	...
	//新增保存
	$("#form1").ajaxSubmit({
       	url:"<%=basePath%>yuan/saveOrUpdate",
    	dataType:'json',
        async:true,
        type:'POST',
        success:function (result) {
        	if(result.result == 1){
		       //刷新表单
		    	$("#data").load("<%=basePath%>yuan/add?id="+result.vo.id,function(){
		    		if(result.vo.parentId == "ROOT"){$("#parentId_tr01").hide();}
		    		initTree();
		    		alert("保存成功");
		    	});
		        return false;
...
function ev_edit(){
	var id = jQuery("#id").val();
	window.showModalDialog("<%=basePath%>yuan/rowList?id="+id,window,"dialogWidth:"+window.screen.width+"px;dialogHeight:"+window.screen.height+"px"); 
}
