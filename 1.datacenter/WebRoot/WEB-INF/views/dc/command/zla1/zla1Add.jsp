...
<head>
	...
	<script type="text/javascript" >var basePath11 = "<%=basePath%>rulesController";var isCcFlag = "0";</script>
...
<form action=""  method="post"  id="form1">
	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="editTable reeditTable" align="center" id="table01">
		...
		<input type="hidden" name="worksType" id="worksType" value="1"/>
		<tr>
			<td colspan="4">
		...
		<tr>
		    ...
		    name="supportorgName"  id="supportorgName"  type="text" class="text01" maxlength="100"  style="width:500px;" placeholder="请输入支撑单位"  value="${vo.supportorgName}"/>
		    <input type="hidden" name="supportorgId"  id="supportorgId"/>
	    ...
	    <tr>
	    	...
	    	<td>
	    	<input name="contractName" id="contractName" <c:if test="${not empty vo.contractName }">readonly="readonly"</c:if> onfocus="autoCompletesProject(this)" type="text" class="text01" style="width:500px;" placeholder="请输入合同名称" value="${vo.contractName }"/>
	    	</td>
	    </tr>
	    <tr>
	    	<th width="20%" align="right"><b>*</b>变更涉及合同编号:</th>
	    	<td>
	    	<!-- c:if 用于判断是否为编辑页面，如果是，则该内容不可编辑 -->
	    	<input name="contractCode" id="contractCode" <c:if test="${not empty vo.contractCode }">readonly="readonly"</c:if> onfocus="autoCompletesProject(this)" type="text" class="text01" style="width:500px;" placeholder="请输入合同编号" value="${vo.contractCode }"/>
	    	</td>
	    </tr>
	    <tr>
	    	<th width="20%" align="right"><b>*</b>施工单位:</th>
	    	<td>
	    	<input name="constructionName" id="constructionName" type="text" class="text01" style="width:500px;" placeholder="请输入施工单位" value="${vo.constructionName }"/>
	    	<input type="hidden" name="constructionId"  id="constructionId"/>
	    	</td>
	    </tr>
	    <tr>
		    <th width="20%" align="right">变更信息：</th>
	    </tr>
	    <tr>
	    ...
	    <tr>
	    	<th width="20%" align="right"><b>*</b>内容:</th>
	    	<td>
	    ...
	    <tr>
	    	<th width="20%" align="right"><b>*</b>原因:</th>
	    	<td>
	    ...
	    <!-- 	    上传附件 -->
	  	<tr id="fi_box_tr_11">
		    ...
		    <td>
		    	<div class="text01" style="width:500px;" id="fi_box_div_11" value="11">
...
function ev_save(){
	...
	$("#form1").ajaxSubmit({
       	url:"<%=basePath%>command/zla1SaveOrUpdate",
    	dataType:'json',
        async:true,
        type:'POST',
...
function ev_required_item(){
		...
		if(bindChange("#contractName",100,"工程名称不得超过100个字")){
			return false;
		}
		if(bindChange("#contractCode",36,"合同编号不得超过36个字")){
			return false;
		}
		if(bindChange("#constructionName",75,"施工单位不得超过75个字")){
			return false;
		}
		if(bindChange("#orgName",90,"部位不得超过90个字")){
			return false;
		}
		if(bindChange("#worksContent",90,"内容不得超过90个字")){
			return false;
		}
