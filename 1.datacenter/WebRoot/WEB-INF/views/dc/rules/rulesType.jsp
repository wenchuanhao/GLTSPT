...
var treeNodeObj;
	...
	function setRenameBtn(treeId, treeNode) {
		...
		//树结构数据初始化(不能用循环)
	var zNodes = $.parseJSON('${zNodes}'.replace(/\r\n/g, "\\r\\n").replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&").replace(/&reg;/g,"®").replace(/&copy;/g,"©").replace(/&trade;/g,"™"));
    ...
    //点击编辑
		...
		var s = jQuery("#busTypes_input").val();
		if(s=="BZ"){
		 if(treeNode.id != "ROOT"){
			jQuery.ajax({
				type:"POST",
				async:false,
				url: "<%=basePath%>rulesController/queryWorkDay?busTypes=${busTypes}",
	...
	<tr>
	    ...
	    <td valign="top">
	    	<table id="table01" cellpadding="0" cellspacing="5" class="taskCreate" width="100%">
	    		...
	    		<input type="hidden" name="busTypesName" id="busTypesName" value="${busTypesName }"/>
	    		<input type="hidden" name="tempName" id="tempName"/>
	    		
	            <tr id="rules_type_tr01">
	               <th width="35%" style="text-align: right"><em>*</em>${busTypesName }一级：</th>
	               <td>
	             ...
	             <tr>
	               <th width="35%" style="text-align: right" id="rules_type_th02"><em>*</em>${busTypesName }二级：</th>
	               <td><input  name="typeName" id="typeName_input"  type="text" class="text01" style="width:174px;" /></td>
	             </tr>
	             <c:if test="${busTypesName=='费用类型' }">
                <tr id="rules_type_th03">
	               <th style="text-align: right"><em>*</em>整改超时工作日：</th>
	               <td><input  name="workDay" id="workDay" type="text" class="text01" style="width:174px;" value="5"/> </td>
           		</tr>
	             </c:if>
	             </form>
	            <tr>
...
function checkSubmit(){
	if( jQuery.trim(jQuery("#parentTypeId_input").val())=="" ){
		animateAlert("table01","sel_01", "请选择${busTypesName }一级" );
		return false;
	}
	...
	if("${busTypesName}"=="费用类型" && $("#workDay").is(":visible")){
		if( jQuery.trim($("#workDay").val())=="" ){
			animateAlert("table01","workDay","整改超时工作日不能为空" );
			return false;
		}else {
			if(isNaN(jQuery.trim($("#workDay").val()))){
				animateAlert("table01","workDay","请输入数字" );
				return false;
			}
		}
	}
...
function checkCostType(){
	...
	$.ajax({
	        type:"POST",
	        async:false,
	        url:"<%=basePath%>rulesController/checkCostType",
	        data:"tempName="+jQuery("#tempName").val()+"&typeName=" + jQuery("#typeName_input").val() + "&parentTypeName="+jQuery("#parentTypeName_input").val()+"&busTypes=${busTypes }",
...
function ev_submit(){
	...
	if(jQuery("#busTypesName").val()=="费用类型"){
		if(checkCostType()=="1"){
			alert("该费用类型已存在，请重新输入");
			return;
		}
	}
	...
	$("#form1").ajaxSubmit({
        ...
        success:function (result) {
        	if(result.result == 1){
				...
				//特殊字符转义
        		result.rulesType.typeName = replaceSpecial(result.rulesType.typeName);
...
//替换特殊字符，js代码无法解析转义字符
	...
	name = name.replace(/&gt;/g,">").replace(/&lt;/g,"<")
			...
			.replace(/&copy;/g,"©").replace(/&trade;/g,"™");
			console.log(name);
	return name;
}
</script>
