				...
				jQuery("#ccUserSearch").autocomplete({
					...
					select: function( event, ui ) {
						if(jQuery("#selectedCCUserDiv").children("[value='" + ui.item.userId + "']").length != 0){
							...
							jQuery("#selectedCCUserDiv").append('<input type="hidden" id="CCUserId" value="' + ui.item.userId + '"/><input type="button" class="wd_cx_bnt01" value="' + ui.item.userName + '" onclick="removeCCUser(\'' + ui.item.userId + '\',\'' + ui.item.userName + '\')"/>');
		...
		success: function(data){
			...
			if(data == "1"){
				...
				if(transitionId==3){//会签 多选
					var temp = "<input type='text' id='userSearch' onfocus='autoCompletes2()' onkeyup='clean1()' style='width:98%'/>";
					jQuery("#ApproveUsersDiv").html(temp);
				}else if (transitionId==4){//平转 单选
				var temp = "<input type='text' id='userSearch' onfocus='autoComplete2()' onkeyup='clean1()' style='width:98%'/>";
					jQuery("#ApproveUsersDiv").html(temp);
				}else{
					var url = "<%=basePath%>developFlowManage/searchVersionApproveUserFromHandleMethods";
					var params = {
				...
				jQuery.post(url, params, function(data){
					var temp = "";
				if(transitionId=='13062309000000000037'){
					    document.getElementById("notetext").innerHTML="请输入处理人";
						temp = "<input type='text' id='userSearch' onfocus='autoComplete()' onkeyup='clean1()' style='width:98%'/>";
						jQuery("#ApproveUsersDiv").html(temp);
				}else{
					//alert(data.length);
...
function autoComplete(){
					...
					source: function( request, response ) {
						jQuery.ajax({
							url: "<%=basePath%>developFlowManage/searchVersionApproveUserFromHandleMethodsload",
							dataType: "json",
							data: {
								userValue: request.term,
								workorderId:jQuery("#developWorkorderId").val(),
