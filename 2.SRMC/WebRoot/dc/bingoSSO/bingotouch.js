...
;ui.Utils = {
    setViewPort: function () {
        ...
        if (/android (\d+\.\d+)/.test(userAgent)) {
            ...
            var curOrt = elem.clientWidth / Math.max(elem.clientHeight, 320) < 1.1 ? "portrait" : "landscape";
            if(userAgent.match(/ipad/i)){
                if(curOrt == 'landscape'){ //横屏
                    viewport = "width=860,user-scalable=no";
                } else{ //
                    viewport = "width=640,user-scalable=no";
                }
            }else if(userAgent.match(/iphone os/i) == "iphone os"){
                if(curOrt == 'landscape'){ //横屏
                    viewport = "width=720,user-scalable=no";
                } else{ //
...
;ui.Page = (function(){
    ...
    */
    ...
    var _removeHistoryDom = function(tag){
        $(tag).remove();
        $("#section_container>*").filter("script,style,aside:not(.noRemove)").remove();
    }
	...
	if(window.devicePlatform == 'android'){
		document.addEventListener("deviceready", function() {
		    ...
		    document.addEventListener("hidekeyboard", function() {
		        ...
		        for (var i = iscrolls.length - 1; i >= 0; i -= 1) {
		            iscrolls[i].wrapper.scrollTop = 0;
		        }
...
;ui.Tab = function(options){
    ...
    var onBeforeToggle = options.onBeforeToggle || function(){return true;};
    var $container = options.container ? $(options.container) : $("body");
    $(".navbar > ul > li ,.navbar table tr td ", $container).unbind('tap').bind('tap', function(e){
            var tabId = $(this).find('[data-role=BTButton]').attr("data-tab");
    		var res = onBeforeToggle(tabId);
    		if(res){
    			$("#"+tabId ).addClass("active").siblings().removeClass('active');
		        $(this).siblings().find('[data-role=BTButton]').removeClass('btn-active');
        		$(this).find('[data-role=BTButton]').addClass('btn-active');
    			...
    			options.onAfterToggle && options.onAfterToggle();
    		}
