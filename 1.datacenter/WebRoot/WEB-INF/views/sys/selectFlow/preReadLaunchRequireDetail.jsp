...
function search(){
	...
	<body class="bg_c_g">
		...
		<input type="hidden" id="luanchWorkorderId" name="luanchWorkorderId" value="${luanchWorkorder.launchWorkorderId}"/>
		<c:choose>
			<c:when test="${empty requestScope.indexLocation }">
				<div class="gl_m_r_nav">当前位置&nbsp;:&nbsp;我的工作台&nbsp;>&nbsp;需求流程&nbsp;>&nbsp;需求详情</div>
			</c:when>
		...
		<div class="gl_import_m">
			...
			<table class="gl_table_a01_6L" width="100%" border="0" cellspacing="0"
				...
				<th width="100">
					...
					<%-- <td>
						${luanchWorkorder.name}
					</td> --%>
					<td title="${luanchWorkorder.name}">
						<%-- ${developWorkorder.name} --%>
						<script>document.write(ellipsisStr3('${luanchWorkorder.name}'));</script>
					</td>
					<th width="100">
						需求类型:
					</th>
					<td>
						<c:if test="${luanchWorkorder.type=='1'}">产品功能</c:if>
						<c:if test="${luanchWorkorder.type=='2'}">运营支撑</c:if>
						<c:if test="${luanchWorkorder.type=='3'}">系统优化</c:if>
						<c:if test="${luanchWorkorder.type=='4'}">营销策划</c:if>
						<c:if test="${luanchWorkorder.type=='5'}">其它</c:if>
					</td>
				</tr>
				<tr>
					<th width="100">
					...
					<td>
						<c:if test="${luanchWorkorder.emergencyDegree=='1'}">低</c:if>
						<c:if test="${luanchWorkorder.emergencyDegree=='2'}">中</c:if>
						<c:if test="${luanchWorkorder.emergencyDegree=='3'}">高</c:if>
						<c:if test="${luanchWorkorder.emergencyDegree=='4'}">极高</c:if>
					</td>
					<th width="100">
						期望完成时间:
					</th>
					<td>
						<fmt:formatDate value="${luanchWorkorder.expectFinishDate}" pattern="yyyy-MM-dd"/>
					</td>
					<td colspan="2"></td>
				</tr>
			</table>
			...
			<table class="gl_table_a01" width="100%" border="0" cellspacing="0"
				...
				<tr>
					...
					<td>
						<textarea style="background-color: #eff4fa;" name="reason" class="gl_text01_d" id="reason" readonly="readonly">${luanchWorkorder.reason}</textarea>
					</td>
				</tr>
				<tr>
					<th width="100">
					...
					<td>
						<textarea style="background-color: #eff4fa;" name="description" class="gl_text01_d" id="description" readonly="readonly">${luanchWorkorder.description}</textarea>
					</td>
				</tr>
				<tr>
					<th width="100">
					...
					<td>
						<textarea style="background-color: #eff4fa;" name="expectBenefits" class="gl_text01_d" id="expectBenefits" readonly="readonly">${luanchWorkorder.expectBenefits}</textarea>
					</td>
				</tr>
			</table>
			<div class="ge_a01"></div>
...
function doSubmit(){
	jQuery("#submitForm").attr("action","<%=basePath%>core/portal/submitRequireReadFlowTwo");
