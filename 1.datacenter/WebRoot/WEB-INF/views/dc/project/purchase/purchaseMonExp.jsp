...
<div class="gl_import_m">
    ...
    <input type="hidden" value="${purchaseForm.pageSize}" id="pageSize"	name="pageSize"/>
  	...
  	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="editTable reeditTable" align="center">
	    ...
	    <td width="30%"  colspan="3">
	    	...
	    	<input readonly="readonly" name="beginCreateTime" id="beginCreateTime" type="text"  placeholder="请输入开始时间"  class="text02 l" 
	    					value="${time}"
	                      	onclick="WdatePicker({startDate:'%y-%M', dateFmt:'yyyy-MM',alwaysUseStartDate:true})"  /><i></i>
	        </div>
		</td>
	</tr>
  	...
  	<div class="tabpages">
    	<ul class="l">
	      		<li class="current" >南方基地项目月份采购统计汇总表<em></em></li>
    	</ul>
    	<div class="otherButtons r">
    			<a class="btn_common01" href="javascript:ev_export()" /><img src="/SRMC/dc/images/btnIcon/export.png" /><span>导 出</span></a>   			
		</div>    	
  	</div>
	<table width="99%" style="min-width: 99%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTable">
	  <tr>
<th rowspan="2">采购方式</th>
<th colspan="4">${month }月</th>
<th colspan="4">年度累计（1-${month}月）</th>
 </tr>
<tr>
<th>项目数量(个)</th>
<th>项目量占比</th>
...
<th>决策额占比</th>
 ...
 <c:forEach items="${crrentMonth}" var="vo" varStatus="i">	  
<tr>
...
function ev_search(){
	document.forms[0].action="<%=basePath%>purchaseExp/purchaseMonQuery?key="+Math.random();
...
function ev_export(){
	document.forms[0].action="<%=basePath%>purchaseExp/exportMonFile?key="+Math.random();
	document.forms[0].submit();
	document.forms[0].action="<%=basePath%>purchaseExp/purchaseMonQuery?key="+Math.random();
}
