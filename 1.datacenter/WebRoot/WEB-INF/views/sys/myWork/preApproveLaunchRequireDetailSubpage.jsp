...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body class="bg_c" id="Bbody">
		<input type="hidden" id="launchWorkorderId" name="launchWorkorderId" value="${launchWorkorderId}"/>
		...
		<div class="" style="padding:5px 10px;">
			...
			<table class="tuan_table_d01" width="100%" border="0" cellspacing="0"
				...
				<tr>
					...
					<td width="250" align="right">
						<div id="uboxstyle">
							<select class="select_app" id="personalOpinionId" name="personalOpinionId" onchange="selectPersonalOpinion()">
								<option value="-1">
									选择个人常用意见
								</option>
			...
			<div id="CCUserDiv" style="display: block;">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						<td>
							<input type="text" class="tuan_text01" id="ccUserSearch" onfocus="autoCompleteCCUser();" onkeyup="clean()"/>
						</td>
						<td>
	...
	jQuery.ajax({
		...
		success: function(data){
			...
			if(data == "1"){
				...
				if(transitionId==3){//会签  多选
					var temp = "<input type='text' onblur='clearText(userSearch);'  id='userSearch' onfocus='autoCompletes()' style='width:98%' onkeyup='clean1()'/>";
					jQuery("#ApproveUsersDiv").html(temp);
				}else if (transitionId==4){//平转标志  单选
				var temp = "<input type='text' onblur='clearText(userSearch);' id='userSearch' onfocus='autoComplete()' onblu style='width:98%' onkeyup='clean1()'/>";
					jQuery("#ApproveUsersDiv").html(temp);
				}else{
					var url = "<%=basePath%>developFlowManage/searchApproveUserFromHandleMethods";
				var params = {
					workorderId:jQuery("#launchWorkorderId").val(),
					flowType:"2",
					transitionId:transitionId,
					flag:"0"
				};
				...
				if(transitionId==3){//会签 多选
					var temp = "<input type='text' id='userSearch' onblur='clearText(userSearch);' onfocus='autoCompletes()' style='width:98%' onkeyup='clean1()'/>";
					jQuery("#ApproveUsersDiv").html(temp);
					
				}else if(transitionId==4){	/*****平转(单选)******/
					var temp = "<input type='text' id='userSearch' onblur='clearText(userSearch);' onfocus='autoComplete()' style='width:98%' onkeyup='clean1()'/>";
					jQuery("#ApproveUsersDiv").html(temp);
				}else{
					var url = "<%=basePath%>developFlowManage/searchApproveUserFromHandleMethods";
					var params = {
...
function doSubmit(){
	...
	//if(jQuery.trim(jQuery("#ccUserSearch").val()) == ""){
		//alert("请选择抄送人");
		//return;
	//}
