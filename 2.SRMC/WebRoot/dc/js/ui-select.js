...
; + function($) {
    ...
    UI_select.prototype = {
        ...
        _setHtml: function() {
            ...
            this.el.wrap('<div tabindex="0" class="ui-select-wrap ' + this._opt.wrapClass + '" id="div_select_'+this.el.attr("id")+'"></div>')
        ...
        add: function(key,value) {
            this.el.append('<option value="'+key+'">'+value+'</option>');
            this._list.append('<li class="" data-value="'+key+'" title="'+value+'">'+value+'</li>');	
        },
...
//            this._input.html(_selectedLi.text()).attr('title', _selectedLi.text());
            ...
            var item = this.items.eq(_selectedLi.index());
            $(_selectedLi).remove();
            $(item).remove();
        },
        //删除全部,并设置默认提示语
        removeAll: function(title) {
        	this._list.children().remove();//删除ul
        	this._input.html(title).attr('title', title);
        	this.items.remove();//删除select
        },
        // 值改变事件；
