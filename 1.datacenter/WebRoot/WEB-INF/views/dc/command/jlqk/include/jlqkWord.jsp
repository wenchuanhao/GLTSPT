...
<div class="contract_wraper">
	<div class="contract" id="contract" style="height:auto;">
		<div class="contract_head">
			<h1 class="contract_title">非采购订单-工程建设类请款审批表<i>（监理、造价）</i></h1>
		...
		-->
            ...
            <tr class="noBorder">
                ...
                <div class="word_5">乙方单位申请事由：
                	...
                	<i class="<c:if test='${empty vo.workContent }'>divcss5-x5-null</c:if><c:if test='${not empty vo.workContent }'>divcss5-x5</c:if>" style="min-width:110px;"><c:if test="${empty vo.workContent }">&nbsp;</c:if><c:if test="${not empty vo.workContent }">${vo.workContent }</c:if></i> 工作内容，根据合同条款
                	<i class="borderBtm" style="min-width:40px;"><c:if test="${empty vo.termsNum }">&nbsp;</c:if><c:if test="${not empty vo.termsNum }">${vo.termsNum }</c:if></i> 条：
                	<i class="<c:if test='${empty vo.termsContent }'>divcss5-x5-null</c:if><c:if test='${not empty vo.termsContent }'>divcss5-x5</c:if>" style="min-width:110px;"><c:if test="${empty vo.termsContent }">&nbsp;</c:if><c:if test="${not empty vo.termsContent }">${vo.termsContent }</c:if></i> 。本期间应扣金额为
           ...
           <tr class="noBorder">
                ...
                <p>经复核，乙方单位<i class="borderBtm" style="width:100px;">&nbsp;</i>年<i class="borderBtm" style="width:40px;">&nbsp;</i>月期间：</p>
                <p>一、已完成工程款额<b class="borderBtm" style="width:100px;">&nbsp;</b>元；</p>
                <p>二、根据合同条款<b class="borderBtm" style="width:100px;">&nbsp;</b>条：<b class="borderBtm" style="width:400px;">&nbsp;</b>。</p>
                <p class="indent">本期应扣金额为<b class="borderBtm" style="width:100px;">&nbsp;</b>元，本次应支付金额<b class="borderBtm" style="width:100px;">&nbsp;</b>元（大写），计算过程：</p>
                </td>
			</tr>
            <tr>
            	<td colspan="2"  class="noBorderR ">签名：</span><input name="" type="text" class="w-text01"></td>
                <td colspan="2"><span class="width100">日期：</span><input name="" type="text" class="w-text01">年<input name="" type="text" class="w-text02">月<input name="" type="text" class="w-text02">日<input name="" type="text" class="w-text02">时</td> 
            </tr>
            <tr class=" noBorder">
				<td colspan="4" class="contract_item" style="text-align:left;">
                <div class="pad-btm10">项目负责人意见/主管意见：</div>
                <div class="contract_chex">
						<span>
