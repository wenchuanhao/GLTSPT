...
<div class="gl_import_m">
    ...
    <input type="hidden" value="${tzlhReport.pageSize}" id="pageSize"	name="pageSize"/>	
  	...
  	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="editTable reeditTable" align="center">
	    ...
	    <td width="30%">
	    	<select class="ui-select" id="dept" name="dept" style="width:200px;">
	    ...
	    <td width="30%">
		    <input  onfocus="onfocuses(this)" 
	                    onblur="onblurs(this)"
	                  name="ks" id= "ks" value="${tzlhReport.ks}" type="text" class="text01" style="width:195px;" placeholder="请填写投资科室"/>
		</td> 
	</tr>
	<tr>
  	...
  	<div class="tabpages">
    	<ul class="l">
	      		<li class="current" >投资经分报表<em></em></li><font color="red" size="3">&nbsp;&nbsp;&nbsp;温馨提示：数据来源于投资管理系统。</font>
    	</ul>
	...
	<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTable">
	  ...
	  <c:forEach items="${ITEMPAGE.items}" var="vo" varStatus="i">
	  	 <tr <c:if test="${i.count % 2 == 1}">class="td01"</c:if> ${vo.b eq null ? 'style=\"background-color: #d6ebf9;\"':''}>
		    ...
		    <!--<td><fmt:formatNumber value="${vo.c}" type="currency"  maxFractionDigits="6"/></td><!-- 计划转资金额汇总（万元） -->
			<!--<td><fmt:formatNumber value="${vo.d}" type="currency"  maxFractionDigits="6"/></td><!-- 实际转资金额汇总（万元） -->
			<!--<td>${vo.e}%</td><!-- 项目转资率 -->
...
//提交查询
function ev_search(){
	document.forms[0].action="<%=basePath%>GCTzlh/index?key="+Math.random();
...
function ev_export(){
	document.forms[0].action="<%=basePath%>GCTzlh/exportFile?key="+Math.random();
	document.forms[0].submit();
	document.forms[0].action="<%=basePath%>GCTzlh/index?key="+Math.random();
}
